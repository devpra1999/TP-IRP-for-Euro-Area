...
f_6 <- read.csv("./Data_Files/Consensus Forecasts Updated/6-months.csv", skip = 6,col.names = c("Date","L2_forecast","Type"))
f_6 <- f_6[order(f_6$Date),]
f_6$Date = as.Date(as.yearqtr(f_6$Date, format = "%YQ%q")) + months(3)
f_9 <- read.csv("./Data_Files/Consensus Forecasts Updated/9-months.csv", skip = 6,col.names = c("Date","L3_forecast","Type"))
f_9 <- f_9[order(f_9$Date),]
f_9$Date = as.Date(as.yearqtr(f_9$Date, format = "%YQ%q")) + months(3)
f_12 <- read.csv("./Data_Files/Consensus Forecasts Updated/12-months.csv", skip = 6,col.names = c("Date","L4_forecast","Type"))
