...
#LOAD PACKAGES AND DEFINE GLOBAL VARIABLES AND FUNCTIONS REQUIRED
library(shiny)
library(highcharter)
lr <- tail(master_df,1)
...
ui <- fluidPage(
    ...
    textOutput("desc2"),
    highchartOutput("rateplot1"),
    fluidRow(
      column(6,numericInput("F1",label = "Forecast for Q1", value = round(lr$Rate))),
      column(6,numericInput("F2",label = "Forecast for Q2", value = round(lr$Rate))),
      column(6,numericInput("F3",label = "Forecast for Q3", value = round(lr$Rate))),
...
server <- function(input, output) {
    ...
    output$rateplot1 <- renderHighchart({
      highchart() %>%
        hc_xAxis(type = "datetime", dateTimeLabelFormats = list(month = "%b %Y")) %>%
    ...
    output$rateplot2 <- renderHighchart({
      highchart() %>%
        hc_xAxis(type = "datetime", dateTimeLabelFormats = list(month = "%b %Y")) %>%
        hc_yAxis(title = list(text = "Interest Rate"), min = -1, max = 5) %>%
        hc_add_series(data = temp, hcaes(x = Date, y = Rate),
