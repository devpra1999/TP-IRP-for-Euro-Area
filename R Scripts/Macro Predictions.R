...
Germany_comp <- master_df %>%
  select(Date,Spread_Germany,TP_cf_Germany,sum_Er_cf, Excess_Return_Germany, Rate) %>%
  mutate(Monetary_Policy = sum_Er_cf + Rate) %>%
  select(Date,Spread_Germany,TP_cf_Germany,Monetary_Policy, Excess_Return_Germany)
colnames(Germany_comp) <- c("Date","Spread","Term_Premia","Monetary_Policy","Excess_Return")
...
France_comp <- master_df %>%
  select(Date, Spread_France, TP_cf_France, sum_Er_cf, Excess_Return_France, Rate) %>%
  mutate(Monetary_Policy = sum_Er_cf + Rate) %>%
  select(Date, Spread_France, TP_cf_France, Monetary_Policy, Excess_Return_France)
colnames(France_comp) <- c("Date", "Spread", "Term_Premia", "Monetary_Policy","Excess_Return")
...
Italy_comp <- master_df %>%
  select(Date, Spread_Italy, TP_cf_Italy, sum_Er_cf, Excess_Return_Italy, Rate) %>%
  mutate(Monetary_Policy = sum_Er_cf + Rate) %>%
  select(Date,Spread_Italy,TP_cf_Italy,Monetary_Policy, Excess_Return_Italy)
colnames(Italy_comp) <- c("Date", "Spread", "Term_Premia", "Monetary_Policy","Excess_Return")
...
Spain_comp <- master_df %>%
  select(Date, Spread_Spain, TP_cf_Spain, sum_Er_cf, Excess_Return_Spain, Rate) %>%
  mutate(Monetary_Policy = sum_Er_cf + Rate) %>%
  select(Date, Spread_Spain, TP_cf_Spain, Monetary_Policy, Excess_Return_Spain)
colnames(Spain_comp) <- c("Date", "Spread", "Term_Premia", "Monetary_Policy","Excess_Return")
...
fe_gr_q <- plm(Growth_q ~ lag(Growth_q,1) + lag(Spread, 1),
               data = macro_table, model = "within", effect = "individual")
fe_gr_q_sep <- plm(Growth_q ~ lag(Growth_q,1) + lag(Monetary_Policy, 1) + lag(Term_Premia, 1),
                   data = macro_table, model = "within", effect = "individual")

#Growth - Year-on-Year
fe_gr_a <- plm(Growth_a ~ lag(Growth_a,4) + lag(Spread, 4),
               data = macro_table, model = "within", effect = "individual")
fe_gr_a_sep <- plm(Growth_a ~ lag(Growth_a,4) + lag(Monetary_Policy, 4) + lag(Term_Premia, 4),
                   data = macro_table, model = "within", effect = "individual")
...
stargazer(fe_inf_q, fe_inf_q_sep, fe_inf, fe_inf_sep, type = "html", out = "inflation.doc")
...
fe_exr <- fe_gr_q <- plm(Excess_Return ~ lag(Spread, 1),
                         data = macro_table, model = "within", effect = "individual")
fe_exr_sep <- plm(Excess_Return ~ lag(Monetary_Policy, 1) + lag(Term_Premia, 1),
                   data = macro_table, model = "within", effect = "individual")
stargazer(fe_exr,fe_exr_sep, type = "html", out = "excess_returns.doc")
